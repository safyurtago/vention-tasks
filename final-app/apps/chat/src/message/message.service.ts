...
export class MessageService {
  ...
  async create(createMessageDto: CreateMessageDto, req: Request) {
    ...
    return this.prismaService.message.create({
      data: { ...createMessageDto, senderId },
    });
  }

  async findAll(req: Request) {
    const senderId = req['user'].id;
    const sendedMessages = await this.prismaService.message.findMany({
      where: { senderId },
    });
    const receivedMessages = await this.prismaService.message.findMany({
  ...
  async findOne(id: string, req: Request) {
    ...
    const senderId = req['user'].id;
    if ((message.senderId || message.receiverId) !== senderId)
      throw new BadRequestException('You do not have any message with this id');
  ...
  async update(id: string, updateMessageDto: UpdateMessageDto, req: Request) {
    ...
    if (message.senderId !== senderId)
      throw new BadRequestException(
        'You do not have permission to update others Messages',
      );
