...
export class UserService {
  ...
  // SIGN UP USER
  async signUp(createUserDto: CreateUserDto, res: Response, file: any) {
    const {
      first_name,
      last_name,
      email,
    ...
    const findUser1 = await this.prismaService.user.findUnique({
      where: { username },
    });
    if (findUser1) {
      throw new BadRequestException(
  ...
  // FIN FILTERED USER
  async findFilteredUsers(findFilteredUsersDto: FindFilteredUsersDto) {
    const where = {};
  ...
  async changePassword(changePasswordDto: ChangePasswordDto, req: Request) {
    try {
      ...
      if (
        ...
        changePasswordDto.confirm_new_password
      ) {
